...
@jit
def line_of_sight(r_sat, r_star, R):
    """Determine if satellite has line of sight to star.
    ...
    -------
    bool
        True if line of sight exists, False if occulted
    """
    # Vector from satellite to star
    delta = r_star - r_sat
    # Minimum distance from central body center to line
    d = norm(np.cross(r_sat, delta)) / norm(delta)
    return d > R
...
def radiation_pressure(t0, state, k, R, C_R, A_over_m, Wdivc_s, star):
    ...
    nu = float(line_of_sight(r_sat, r_star, R))
